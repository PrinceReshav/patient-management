        ...
        KafkaProducer kafkaProducer)
{
    ...
    @CircuitBreaker(name="billingService" , fallbackMethod = "billingFallback") // gets triggered when retry fails
    @Retry(name = "billingRetry") // if this fails circuitBreaker pattern kick in and circuit is open
    ...
    public BillingResponse billingFallback(String patientId, String name, String email, Throwable t) {
        log.warn("[CIRCUIT BREAKER]: Billing service is unavailable. Triggered fallback: {}",
                t.getMessage());
